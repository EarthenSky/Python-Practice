fix_str = """kAewtloYgcFQaJNhHVGxXDiQmzjfcpYbzxlWrVcqsmUbCunkfxZWDZjUZMiGqhRRiUvGmYmvnJIHEmbT
...
fix_str = fix_str.replace(" ", "")
fix_str = fix_str.replace("\r", "")

#-----------------------------------------------
'''

y = fix_str
cut_string = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"
...
out_str = ""
for char in fix_str:
    # First Body guards, Second Body guards.
    if len(out_str) == 1 or len(out_str) == 2 or len(out_str) == 3 or len(out_str) == 5 or len(out_str) == 6 or len(out_str) == 7:
        if char.isupper() == True:
            out_str += char
        else:
            out_str = ""

    elif len(out_str) == 0 or len(out_str) == 4 or len(out_str) == 8:  # Middle character.
...
'''
# First Body guards, Second Body guards.
if len(out_str) == 0 or len(out_str) == 1 or len(out_str) == 2 or len(out_str) == 4 or len(out_str) == 5 or len(out_str) == 6:
    if char.isupper() == True:
        out_str += char
    else:
        out_str = ""

elif len(out_str) == 3:  # Middle character.
    if char.isupper() == False:
        out_str += char
    else:
        out_str = ""
...
#-----------------------------------------------------
#print re.match(r'[a-c]', fix_str)

expr = r'[a-z][A-Z][A-Z][A-Z][a-z][A-Z][A-Z][A-Z][a-z]'

'''
if re.search(expr, fix_str):

    match = re.search(expr, fix_str)

    print("Match at index %s, %s" % (match.start(), match.end()))

'''

guards = re.compile(expr)
print guards.findall(fix_str)

#linkedlist


#PROBLEM -- Halfway done pattern matches didn't go to the next availible letter, they skippped some. and missed part of the word.
